...
- **섹션 리스트 확인**:
  ```bash
  curl https://YOUR_PUBLIC_IP:8443/command
  ```
- **특정 메모 상세 확인**:
  ```bash
  curl https://YOUR_PUBLIC_IP:8443/command/1
  ```
...
- **메모 추가**:
  ...
  curl -X POST \
       ...
       -d '{"content":"메모 내용"}' \
       https://YOUR_PUBLIC_IP:8443/api/memos/command
  ```
...
- **When incoming requests match**:
  - `Hostname` equals `YOUR_PUBLIC_IP`
  - **AND** `SSL/HTTPS` equals `Off` (매우 중요: 루프 방지)
- **Then...**:
  - **URL Redirect**: `Dynamic`
  - **Expression**: `concat("https://YOUR_PUBLIC_IP:2096", http.request.uri.path)`
  - **Status Code**: `301`

### 2. SSL/TLS 모드
- `YOUR_PUBLIC_IP` 도메인에 대해 **Full (Strict)** 모드를 사용하세요.
